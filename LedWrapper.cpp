 ...
 */
boolean LedWrapper::initialize(uint8_t numLeds, uint8_t intensity)
{
	...
	{
		ledController = &FastLED.addLeds<MY_CONTROLLER, MY_LED_PIN>(leds, numLeds).setCorrection(MY_COLOR_CORRECTION);
		status = true;
	}
 ...
 *
 */
CRGB LedWrapper::getPixel(int16_t index)
{
 ...
 *
 */
void LedWrapper::setPixel(int16_t index, CRGB color, uint8_t s)
{
	if( index <0 || index >= ledController->size() )
	{
	    leds[index] = color;
 ...
 */
uint8_t LedWrapper::getIntensity()
{
	return intensity;
 ...
 *
 */
void LedWrapper::setIntensity(uint8_t i)
{
	intensity = i;
 ...
 */
void LedWrapper::fill(CRGB color, uint8_t s)
{
	resetIntensity();
 ...
 *
 */
void LedWrapper::fillPattern(uint8_t pattern, CRGB onColor, CRGB offColor)
{
	resetIntensity();
 ...
 */
void LedWrapper::wipe(uint16_t repeat, uint32_t duration, uint8_t direction, CRGB onColor, CRGB offColor, uint32_t onTime, uint32_t offTime, uint8_t clearAfter, uint8_t clearEnd)
{
	uint16_t count = 0;
 ...
 */
void LedWrapper::rotatePattern(uint16_t repeat, uint32_t duration, uint8_t pattern, uint8_t direction, CRGB onColor, CRGB offColor, uint32_t onTime, uint32_t offTime)
{
	uint16_t i = 0;
	uint16_t count = 0;
	uint32_t endTime = millis() + duration;
 ...
 * @clearEnd - clear after complete
 */
void LedWrapper::scrollPattern(uint16_t repeat, uint32_t duration, uint8_t pattern, uint8_t patternLength, uint8_t direction, CRGB onColor, CRGB offColor, uint32_t onTime, uint32_t offTime, uint8_t clearAfter, uint8_t clearEnd)
{
	uint16_t count = 0;
	uint32_t endTime = millis() + duration;
 ...
 *
 */
void LedWrapper::bounce(uint16_t repeat, uint32_t duration, uint8_t pattern, uint8_t patternLength, uint8_t direction, CRGB onColor, CRGB offColor, uint32_t onTime, uint32_t offTime, uint32_t bounceTime, uint8_t clearAfter, uint8_t clearEnd)
{
 ...
 */
void LedWrapper::middle(uint16_t repeat, uint32_t duration, uint8_t direction, CRGB onColor, CRGB offColor, uint32_t onTime, uint32_t offTime, uint8_t clearAfter, uint8_t clearEnd)
{
	uint16_t count = 0;
	uint32_t endTime = millis() + duration;;
 ...
 */
void LedWrapper::randomFlash(uint16_t repeat, uint32_t duration, uint32_t onTime, uint32_t offTime, CRGB onColor, CRGB offColor, uint8_t number)
{
	uint16_t count = 0;
	uint32_t endTime = millis() + duration;;
 ...
 */
void LedWrapper::fade(uint8_t direction, uint8_t fadeIncrement, uint32_t time, CRGB color)
{
	int16_t i=0;
 ...
 * Flashes LEDs
 */
void LedWrapper::strobe(uint16_t repeat, uint32_t duration, CRGB onColor, CRGB offColor, uint32_t onTime, uint32_t offTime )
{
	uint16_t count = 0;
	uint32_t endTime = millis() + duration;;
 ...
 */
void LedWrapper::lightning(uint16_t repeat, uint32_t duration, CRGB onColor, CRGB offColor)
{
	uint16_t count = 0;
	uint32_t endTime = millis() + duration;;
 ...
 */
void LedWrapper::rainbow(uint32_t duration, uint8_t glitterProbability, CRGB glitterColor, uint32_t onTime, uint8_t hueUpdateTime)
{
	uint8_t hue = 0;
	uint32_t hueTime = 0;
	uint32_t endTime = millis() + duration;
 ...
 *
 */
void LedWrapper::rainbowFade(uint32_t duration, uint32_t onTime)
{
	uint32_t endTime = millis() + duration;;
 ...
 *
 */
void LedWrapper::confetti(uint32_t duration, CRGB color, uint8_t fadeBy, uint32_t onTime, uint8_t hueUpdateTime)
{
	uint8_t hue = 0;
	uint32_t hueTime = 0;
	uint32_t endTime = millis() + duration;;
 ...
 *
 */
void LedWrapper::cylon(uint16_t repeat, uint32_t duration, CRGB color, uint32_t fadeTime, uint8_t fps, uint8_t hueUpdateTime)
{
	uint8_t hue = 0;
	uint32_t hueTime = 0;
	uint16_t count = 0;
 ...
 *
 */
void LedWrapper::bpm(uint32_t duration, uint32_t onTime, uint8_t hueUpdateTime)
{
	uint8_t hue = 0;
	uint32_t hueTime = 0;
	uint32_t endTime = millis() + duration;;
 ...
 *
 */
void LedWrapper::juggle(uint32_t duration, uint32_t onTime)
{
	uint32_t endTime = millis() + duration;;
 ...
 *
 */
void LedWrapper::stack(uint16_t repeat, uint32_t duration, uint8_t direction, CRGB onColor, CRGB offColor, uint32_t onTime, uint8_t clearEnd)
{
	uint8_t index;
	uint16_t count;
	uint32_t endTime;
 ...
 *
 */
void LedWrapper::fillRandom(uint16_t repeat, uint32_t duration, CRGB onColor, CRGB offColor, uint32_t onTime, uint32_t offTime, uint8_t clearAfter, uint8_t clearEnd)
{
	uint8_t total;
	uint8_t index;
	uint16_t count;
 ...
 */
void LedWrapper::setPatternTimed(int16_t startIndex, uint8_t pattern, CRGB onColor, CRGB offColor, uint32_t onTime, uint32_t offTime, uint8_t clearAfter)
{
	CRGB currentColor[8];
 ...
 * is true, returns color to original color and waits offTime before returning.
 */
void LedWrapper::setPixelTimed(int16_t index, CRGB newColor, uint32_t onTime, uint32_t offTime, uint8_t clearAfter)
{
    CRGB curColor;
 ...
 *
 */
void LedWrapper::setPattern(int16_t startIndex, uint8_t length, uint8_t pattern, uint8_t patternLength, CRGB onColor, CRGB offColor, uint8_t s)
{
    int16_t index;
 ...
 */
void LedWrapper::setPixel(int16_t index, CRGB color)
{
	if( index <0 || index >= ledController->size() )
	{
	    leds[index] = color;
 ...
 *
 */
void LedWrapper::resetIntensity()
{
	if(intensity == 0 )
	{
		intensity = DEFAULT_INTENSITY;
